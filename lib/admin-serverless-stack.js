...
class AdminStack extends cdk.Stack {
  ...
  constructor(scope, id, props) {
    ...
    //DynamoDB
    const adminServiceTable = new dynamoDB.Table(this, "admin-service", {
    ...
    role.addToPolicy(
      ...
      new iam.PolicyStatement({
        actions: ["dynamodb:*"],
        resources: [
          adminServiceTable.tableArn,
          sessionTable.tableArn,
          new cdk.CfnParameter(
            this,
            "ConnectionTableARN" //Websocket Connection Table ARN
          ).value.toString(),
          new cdk.CfnParameter(
            this,
            "ConnectionTableGSIArn" //Websocket Connection Table GSI ARN
          ).value.toString()
        ]
      })
    );
