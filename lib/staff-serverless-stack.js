...
class StaffServiceStack extends cdk.Stack {
  ...
  constructor(scope, id, props) {
    ...
    //Cognito
    const staffPool = new cognito.UserPool(this, "TROUVER_STAFF_USER_POOL", {
    ...
    const staffSignUpTokenTable = new dynamoDB.Table(
      this,
      "staff-signup-token-details",
      {
        tableName: "staff-signup-token-details",
        partitionKey: {
          name: "email",
          type: AttributeType.STRING
        },
    ...
    const role = new iam.Role(this, "StaffLambdaExecutionRole", {
    ...
    const cognitoTriggerRole = new iam.Role(
      this,
      "StaffTriggerLambdaExecutionRole",
      {
        assumedBy: new iam.ServicePrincipal("lambda.amazonaws.com")
      }
    );
    ...
    cognitoTriggerRole.addToPolicy(
      ...
      new iam.PolicyStatement({
        actions: ["dynamodb:*"],
        resources: [staffSignUpTokenTable.tableArn]
      })
    );
    const cognitoTrigger = new lambda.Function(
      this,
      "staff-service-cognito-trigger-function",
      {
        runtime: lambda.Runtime.NODEJS_12_X,
        handler: "index.handler",
        description: "Lambda function for Staff backend service.",
        functionName: "staff-service-cognito-trigger-function",
        code: lambda.Code.inline(
          `exports.handler = function(event, context, callback) { return callback(null, "hello world"); }`
        ),
        timeout: duration.seconds(30),
    ...
    staffPool.addTrigger(userPoolOperation.CUSTOM_MESSAGE, cognitoTrigger);
